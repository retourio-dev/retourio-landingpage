...
export default function ROICalculator() {
  // Realistische Standardwerte für den Start
  const [returnCount, setReturnCount] = useState(40); 
  ...
  // Marketing Logik: Startup Plan (29€) Vergleich
  ...
  return (
    <section className="py-12 md:py-24 px-4 md:px-6">
      <div className="max-w-6xl mx-auto bg-slate-950 rounded-[2.5rem] md:rounded-[4rem] p-6 md:p-20 text-white shadow-2xl relative overflow-hidden border border-white/5">
        ...
        {/* FIX: Button Positionierung für Mobile (top-4 statt top-6) */}
        <button 
          onClick={() => setShowSettings(!showSettings)}
          className="absolute top-4 right-4 md:top-10 md:right-10 z-20 flex items-center gap-2 bg-white/10 hover:bg-white/20 px-4 py-2 rounded-full transition-all border border-white/10 group"
        >
        ...
        {/* FIX: pt-12 auf Mobile, damit Überschrift nicht unter dem Button liegt */}
        <div className="grid lg:grid-cols-2 gap-10 md:gap-20 items-center relative z-10 pt-8 md:pt-0">
          ...
          <div>
            ...
            {showSettings ? (
              <div className="bg-white/5 border border-white/10 p-5 rounded-3xl mb-8 space-y-6 animate-in fade-in slide-in-from-top-4 duration-300">
                <div>
                  <label className="text-[9px] font-black uppercase text-slate-500 block mb-3 tracking-widest text-center md:text-left">Stundensatz Mitarbeiter (€)</label>
                ...
                <div>
                  <label className="text-[9px] font-black uppercase text-slate-500 block mb-3 tracking-widest text-center md:text-left">Minuten pro Retoure</label>
                  <div className="flex items-center gap-4">
                    <input 
          ...
          <div className="flex flex-col gap-4 md:gap-6">
            <div className="bg-white/5 backdrop-blur-md p-6 md:p-10 rounded-[2rem] md:rounded-[2.5rem] border border-white/10 group w-full transition-all">
            ...
            <div className="bg-white/5 backdrop-blur-md p-6 md:p-10 rounded-[2rem] md:rounded-[2.5rem] border border-white/10 group w-full transition-all">
              <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 md:gap-6 text-center sm:text-left">
                ...
                <div>
                  ...
                  <p className="text-3xl md:text-5xl font-black tracking-tighter tabular-nums text-green-400">{moneySavedYear}€ <span className="text-xs md:text-lg text-slate-400 font-bold uppercase tracking-normal">/ Jahr</span></p>
                </div>
              </div>
            </div>

            <div className="bg-blue-600 rounded-[2rem] p-6 flex items-center justify-between shadow-xl shadow-blue-500/20">
              <div className="flex items-center gap-4">
                <div className="bg-white/20 p-2 rounded-lg text-white shrink-0">
                  <Sparkles size={18} fill="currentColor" />
                </div>
                <p className="text-[10px] md:text-xs font-black uppercase tracking-tight leading-tight">
                  Startup-Plan lohnt sich <br />
                  ab <span className="text-yellow-300 underline">{breakEvenReturns} Retouren/Mo</span>.
                </p>
              </div>
              <a href="/#pricing" className="hidden sm:block bg-white text-blue-600 px-4 py-2 rounded-full text-[9px] font-black uppercase tracking-widest hover:bg-slate-100 transition-colors">
                Details
              </a>
            </div>
