...
export default function Pricing() {
  ...
  const [hoveredIndex, setHoveredIndex] = useState(1); // Standardmäßig Startup (Index 1) markiert
  ...
  const handleCheckout = async (priceId) => {
    ...
    try {
      ...
      if (data.url) {
        ...
        alert("Fehler: Stripe konnte nicht geladen werden. Prüfe deine Vercel Environment Variables.");
        console.error('Stripe Error:', data.error);
      }
    } catch (err) {
      console.error('Checkout Fehler:', err);
      alert("Netzwerkfehler beim Aufruf des Checkouts.");
    } finally {
      setLoadingId(null);
    }
  };
  ...
  return (
    <section id="pricing" className="py-40 px-6">
      <div className="max-w-7xl mx-auto text-center">
        ...
        <h2 className="text-5xl md:text-7xl font-black tracking-tighter mt-6 mb-24 uppercase italic">
          Ready to Scale?
        </h2>
          ...
          {plans.map((p, i) => {
            ...
            return (
              <div
                ...
                className={`p-14 rounded-[4rem] relative transition-all duration-500 border-2 cursor-pointer ${
                  isHovered
              ...
              >
                ...
                <h3 className={`text-[10px] font-black uppercase tracking-[0.3em] mb-6 ${isHovered ? 'opacity-60' : 'opacity-40'}`}>
                  {p.name}
                </h3>
                ...
                <ul className="space-y-6 mb-14 text-left">
                  {p.features.map((f, index) => (
                    <li
                      key={index}
                      className="flex items-center gap-4 text-xs font-bold uppercase tracking-tighter"
                    >
                      <CheckCircle2
                        size={18}
                ...
                <button
                  ...
                  className={`w-full py-6 rounded-2xl font-black uppercase text-[10px] tracking-[0.3em] transition-all active:scale-95 flex justify-center items-center gap-2 ${
                    isHovered
